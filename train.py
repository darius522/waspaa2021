...
with torch.cuda.device(device):
    ...
    #         print(mb.size(), mbl.size(), mbr.size())
            y = m(torch.cat((torch.randn((BStail, 1, zeropadding))*scale, mb), 2)) #896 for W=32
            y = y.to(device)
            mb = mb.to(device)
    ...
    #         err=(-SNR(mbl.squeeze(), yl.squeeze())-SNR(mbr.squeeze(), yr.squeeze()))/2
            ...
            #     with torch.no_grad():
            #         valSNR=FCN_AE_test(m, vPaths, scale, zeropadding, quantization=False, writeout=False)
            #     if valSNR > maxValSNR:
            #         maxValSNR = valSNR
